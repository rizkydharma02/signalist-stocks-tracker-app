...
const CountrySelect = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="bg-gray-800 p-0 border-gray-600 w-full" align="start">
          ...
          <CommandList className="bg-gray-800 max-h-60 scrollbar-hide-default">
            <CommandGroup className="bg-gray-800">
              {countries.map((country) => (
                ...
                >
                  <Check className={cn('mr-2 w-4 h-4 text-yellow-500', value === country.value ? 'opacity-100' : 'opacity-0')} />
                  <span className="flex items-center gap-2">
...
export const CountrySelectField = ({ name, label, control, error, required = false }: CountrySelectProps) => {
  return (
    <div className="space-y-2">
      ...
      <p className="text-gray-500 text-xs">Helps us show market data and news relevant to you.</p>
    </div>
  );
};
